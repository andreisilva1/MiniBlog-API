...
async def get_user(current_user: UserDep):
    ...
    raise HTTPException(
        ...
        detail="No user authenticated. Why not create a new account and login?",
    )
...
async def update_user(
    id: UUID, user: UpdateUser, service: UserServiceDep, current_user: UserDep
) -> User:
    if id != current_user.id:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="You can't edit the profile of another user.",
        )
