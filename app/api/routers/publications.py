...
from app.api.schemas.publication import CreatePublication, ReadPublication, UpdatePublication
...
@router.patch("/")
async def update_publication(current_user: UserDep, id: int, update_publication: UpdatePublication, service: PublicationServiceDep, session: SessionDep):
   ...
   return ReadPublication(**publication.model_dump(exclude=["published_at", "creator", "last_update_at"]), creator_name=creator.nickname, published_at=humanize.naturaltime(datetime.now() - publication.published_at), last_update_at=humanize.naturaltime(publication.last_update_at))

@router.delete("/")
async def delete_publication(id: int, current_user: UserDep, service: PublicationServiceDep):
...
async def convert_publication_to_readable_publication(publications: List[Publication], session: SessionDep):
    ...
    for publication in publications:
        creator = await session.get(entity=User, ident=publication.creator_id)
        result.append(ReadPublication(**publication.model_dump(exclude=["published_at", "creator", "last_update_at"]), creator_name=creator.nickname, published_at=humanize.naturaltime(datetime.now() - publication.published_at), last_update_at=humanize.naturaltime(publication.last_update_at)))
