...
class PublicationService:
    ...
    async def add(self, create_publication: CreatePublication, current_user: User) -> Publication:
        publication = Publication(**create_publication.model_dump(), creator_id=current_user.id, published_at=datetime.now(), creator=current_user)
    ...
    async def get_latest_publications(self):
        latest_publications = await self.session.execute(select(Publication).order_by(desc(Publication.published_at)).limit(1))
        return latest_publications.scalars().all()
    
    async def update(self, id: int, current_user: User, publication_update: UpdatePublication):
        publication = await self.get(id)
        if not publication:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="No post found with the id provided.")
    ...
    async def delete(self, id: int, current_user: User):
        ...
        if not publication or not current_user:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="A error occurred. Verify if you are authenticated and provided a valid publication id.")
        if publication.creator_id == current_user.id:
            await self.session.delete(await self.get(publication.id))
            await self.session.commit()
            return {"detail": f"The publication with the id #{publication.id} has been deleted."}

        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="You cannot delete a post that hasn't been created by you.")
